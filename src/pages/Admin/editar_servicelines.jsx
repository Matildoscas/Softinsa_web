...
function normalizarServiceLine(sl) {
  return {
    ...
    areas: Array.isArray(sl.areas)
      ? sl.areas.map(normalizarArea)
      : [],
  };
}
...
function MultiSelectDropdown({ options, selected, onChange, erro }) {
  ...
  const filtradas = options.filter((o) => {
    const jaSelecionada = selected.some(
...
function EditarServiceLine() {
  ...
  async function carregarDados() {
    try {
      ...
      const listaAreas = Array.isArray(areasData)
        ? areasData
      ...
      setAreasDisponiveis(listaAreas.map(normalizarArea));

      const dadosSL = serviceLineRes.data?.serviceLine || serviceLineRes.data;

      console.log("SERVICE LINE PARA EDITAR:", dadosSL);

      setForm(normalizarServiceLine(dadosSL));
    } catch (err) {
      console.error("Erro ao carregar Service Line:", err);
      console.error("STATUS:", err.response?.status);
      console.error("BODY:", err.response?.data);

      setErroGeral(
        err.response?.data?.error ||
          "Não foi possível carregar a Service Line."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function handleGuardar() {
    ...
    try {
      ...
      setSucesso("Service Line atualizada com sucesso.");
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          ...
          <div style={pageHeader}>
            <div>
              <h5 style={pageTitle}>Editar Service Line</h5>

              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>
                {form.id ? `Service Line #${form.id}` : "A carregar..."}
              </div>
            </div>
          </div>
          ...
          {isLoading ? (
            <div style={loadingBox}>A carregar Service Line...</div>
          ) : (
            <>
              <div style={formCard}>
...
export default EditarServiceLine;